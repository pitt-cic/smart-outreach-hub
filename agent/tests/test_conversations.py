...
from test_utils.database import create_customer, add_campaign_message, initialize_database, cleanup_test_data, add_message_to_history
...
async def get_agent_response_via_process_message(phone_number: str, message: str, message_id: str):
    """
...
async def run_conversation_test(test: ConversationTest, cleanup: bool = True) -> ConversationTest:
    ...
    try:
        ...
        for i in range(len(test.user_messages)):
            ...
            response_wrapper = await get_agent_response_via_process_message(test.phone_number, user_message, message_id)
            
            if not response_wrapper['success']:
                raise Exception(f"Failed to get agent response for message {message_number}: {response_wrapper.get('error', 'Unknown error')}")
            ...
            if agent_response:
                ...
                print(f"  Agent Response {message_number}: {agent_response.response_text}")
                print(f"    Handoff Required: {agent_response.should_handoff}{' (Reason: ' + agent_response.handoff_reason + ')' if agent_response.should_handoff else ''}")
...
async def run_all_tests(limit: int = None) -> List[ConversationTest]:
    ...
    print(f"📋 Testing {num_tests} conversation flow{'' if num_tests == 1 else 's'} in parallel")
    print("=" * 60)
    print()
    ...
    results = await asyncio.gather(*[run_conversation_test(test) for test in tests_to_run])
...
def save_results_json(results: List[ConversationTest], filename: str = None):
    ...
    average_response_tokens_per_message = aggregate_tokens(results)
    ...
    for test in results:
        json_data["tests"].append({
            ...
            "request_tokens": sum(test.results[i]["request_tokens"] for i in range(len(test.results))),
            "response_tokens": sum(test.results[i]["response_tokens"] for i in range(len(test.results))),
        })
...
def aggregate_tokens(results: List[ConversationTest]) -> tuple[int, int, int, int]:
    ...
    total_request_tokens = sum(r.results[i]["request_tokens"] for r in results for i in range(len(r.results)))
    total_response_tokens = sum(r.results[i]["response_tokens"] for r in results for i in range(len(r.results)))

    # Average tokens per input message
    average_request_tokens_per_message = round(total_request_tokens / sum(len(r.results) for r in results))
    average_response_tokens_per_message = round(total_response_tokens / sum(len(r.results) for r in results))

    return total_request_tokens, total_response_tokens, average_request_tokens_per_message, average_response_tokens_per_message
...
def print_test_summary(results: List[ConversationTest]):
    ...
    print(f"⏱️  Average Duration: {sum(r.duration for r in results) / len(results):.2f}s")
    print()
