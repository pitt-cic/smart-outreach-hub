...
async def process_message(phone_number: str, incoming_message: str, incoming_message_id: Optional[str] = None) -> Union[AgentResponseWrapper, None]:
    """
    ...
    try:
        ...
        if not is_valid:
            update_message_attributes(incoming_message_id, attributes={
                'guardrails_intervened': True,
                'user_sentiment': 'negative',
            })
            return AgentResponseWrapper(response_text=guardrails_response, should_handoff=False, guardrails_intervened=True)
        ...
        if customer['status'] != 'automated':
            logfire.info(f"Customer status is {customer['status']}, not responding with AI")
            return None

        # Check if customer has a campaign ID - required for AI processing
        campaign_id = customer.get('most_recent_campaign_id')
        if not campaign_id:
            logfire.info("Customer has no campaign ID, skipping AI processing", phone_number=normalized_phone)
            return None

        # Get campaign-scoped conversation history (exclude current message) and convert to Pydantic AI message format
        conversation_history = get_conversation_history(normalized_phone, campaign_id)[:-1]
        ...
        if agent_response.user_sentiment:
            update_message_attributes(incoming_message_id, attributes={'user_sentiment': agent_response.user_sentiment})
