...
def get_conversation_history(phone_number: str, campaign_id: str) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"Retrieved {len(messages)} messages for {normalized_phone} in campaign {campaign_id}")
        return messages

    except ClientError as e:
        logger.error(f"Error retrieving conversation history for {normalized_phone} in campaign {campaign_id}: {e}")
        return []
    except Exception as e:
        logger.error(f"Unexpected error retrieving conversation history: {e}")
        return []

def get_or_create_customer(phone_number: str, first_name: str = "Unknown", 
                          last_name: str = "Customer") -> Dict:
...
def update_message_attributes(message_id: str, attributes: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(f"Unexpected error updating attributes for message {message_id}: {e}")
        return False
...
def get_last_outbound_message_info(phone_number: str) -> Optional[str]:
    ...
    response = chat_table.query(
        ...
        FilterExpression=Attr('direction').eq('outbound') & Attr('response_type').eq('ai_agent'),
        Limit=1
    )
