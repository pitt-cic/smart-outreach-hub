...
import { CampaignCustomerModel, CampaignModel, CustomerModel } from '../../shared/dynamodb';
...
import { CSVRow, ContactInput, UploadResult } from './types';

// Handle contact upload CSV
export const handleContactUpload = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    try {
        ...
        for (let i = 0; i < csvData.length; i++) {
            ...
            const isDuplicate = validContacts.some((contact) => contact.phoneNumber === normalizedPhone);
        ...
        for (const contact of validContacts) {
            try {
                ...
                errors.push(
                    `Failed to process ${contact.phoneNumber}: ${
